...
class DeepSCAMsModel(Model):
    ...
    def validation(self):
        ...
        test_performance = Validation(
            self.Deep_SCAMS_model, X_test, self.Y_test, "Test"
        ).results
        ...
        validation_performance = Validation(
            self.Deep_SCAMS_model, X_validation, self.Y_validation, "Validation"
        ).results
        self.validation_performance = validation_performance


def create_keras_model(shape=2255, dropout=0.4):
    model = Sequential()
    model.add(
        Dense(500, input_dim=shape, kernel_initializer="normal", activation="relu")
    )
...
class TensorFlowModel(Model):
    ...
    def validation(self):
        ...
        test_performance = Validation(self.TFModel, X_test, self.Y_test, "Test").results
        self.test_performance = test_performance

        X_validation = self.format_for_model(self.scaler, self.X_validation)
        validation_performance = Validation(
            self.TFModel, X_validation, self.Y_validation, "Validation"
        ).results
        self.validation_performance = validation_performance
...
class ALModel(Model):
    def __init__(
        ...
        self.pipe = PipelineSW([("scaler", self.scaler), ("tf_mlp", self.model)])
        self.initialize_al()
    ...
    def initialize_al(self):
        ...
        while (
            len(set(self.Y_train[initial_idx])) != 2
        ):  # Check if both classes are presented
            initial_idx = self.random_choice(self.X_train, self.n_initial)
        ...
        performance_test = Validation(learner, self.X_test, self.Y_test, "Test").results
        ...
        performance_validation = Validation(
            learner, self.X_validation, self.Y_validation, "Validation"
        ).results
        ...
        return (
            ...
            pool,
            performance_test_l,
            performance_validation_l,
            learner,
            [gmean_test, gmean_validation],
        )
    ...
    def run(self):
        ...
        for i in range(self.n_queries - 1):
            ...
            performance_t_iter = Validation(
                learner, self.X_test, self.Y_test, "Test"
            ).results
        ...
        self.test_performance = pd.DataFrame(
            [self.test_performance], index=["test performance"], columns=perf_columns
        )
        ...
        self.validation_performance = list(
            np.max(np.array(performance_validation_l), axis=0)
        )
        self.validation_performance = pd.DataFrame(
            [self.validation_performance],
            index=["validation performance"],
            columns=perf_columns,
        )
        integral_performace = np.concatenate(
            (performance_test_l, performance_validation_l), axis=1
        )
        index_max_pef = np.argmax(gmean(performance_test_l, axis=1))
        final_X, final_Y = (
            X[0 : index_max_pef + self.n_initial,],
            Y[0 : index_max_pef + self.n_initial,],
        )
        self.final_model = TensorFlowModel(
            final_X,
            final_Y,
            self.X_test,
            self.Y_test,
