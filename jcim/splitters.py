...
class BaseSplitter:
    def __init__(self, X, Y, SMILES, mols, ID, type, test_split_r=0.3):
        self.X = X
        self.Y = Y
        self.SMILES = SMILES
        self.mols = mols
...
class TTSSplitter(BaseSplitter):
    def __init__(self, X, Y, SMILES, mols, ID, type, test_split_r=0.3):
        super().__init__(X, Y, SMILES, mols, ID, type, test_split_r=test_split_r)

    def split(self):
        split = train_test_split(self.X, self.Y, test_size=self.test_split_r)
...
class BSplitter(BaseSplitter):
    ...
    def split(self):
        ...
        test_set = dataset_w_mol_obj[
            dataset_w_mol_obj.cluster.isin(uniq_cluster_ids)
        ].sample(n_samples_test)
        train_set = dataset_w_mol_obj[
            ~dataset_w_mol_obj["ID"].isin(test_set["ID"].tolist())
        ]
